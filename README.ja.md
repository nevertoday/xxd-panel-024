...
リポジトリには将来の作例用に [`assets/examples/`](assets/examples/) を用意しています。プロジェクト作者が確認した 024 の完成作品だけを追加し、それまでは別スタイルの投稿や画像を代用しません。

将来の作例は 024 の応用範囲を示すだけで、主体、比喩、配色、文言、画角が生成参照や既定値になることはありません。

## 4つの出力を支えるひとつの幾何感情窓ロジック
...
| モード | サイズ方針 | 成果物 |
| --- | --- | --- |
| `top-bottom` | 元画像適応 | 上に完全な元写真、下に 024 の現実主体が淡色幾何窓を通るデザイン。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `left-right` | 元画像適応 | 左に完全な元写真、右に 024 の現実主体が淡色幾何窓を通るデザイン。両パネルは元画像サイズを保ち、厳密に 50/50 |
| `design-only` | 元画像適応 | 元写真を見せず、変化デザインだけを表示。元画像の比率と寸法を維持 |
| `wallpaper-pack` | 端末別4サイズ | スマートフォン、iPad、デスクトップ、子ども用ウォッチの PNG を個別出力 |

優先順位は、正確な指定ピクセル > 指定比率／用途 > 通常モードの元画像適応です。原稿 `024.md` の 3:4 は初期の制作画角であり、現在の暗黙の既定値ではありません。
...
- **独立4作品：** 各端末は元写真だけを参照し、窓方向、主体通過、淡色窓、余白、薄い補助層、文字関係をそれぞれ探れます。
...
画像モデルが写真に結びついた現実主体、細長い淡色窓、一度の局所通過、本来の素材と色、東洋の余白、薄い補助層、高級商業編集文字を作ります。`scripts/compose_panel.py` は画布計画、厳密な 50/50 ラスター合成、最終寸法、監査だけを担当し、プログラム描画で作品を偽装しません。
