---
name: xxd-panel-024
description: "Create XXD Panel 024 raster artwork while treating the bundled original style brief as the sole aesthetic authority. Supports one or more top-bottom, left-right, design-only, and four-device wallpaper outputs; multiple ratios or exact sizes; prompt-generated, user-exact, or text-free typography; and inline parameters. Use when the user invokes xxd-panel-024 or requests the exact Panel 024 style."
---

# XXD Panel 024

Create fresh PNG artwork with `references/024-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/024-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-024 <source> --mode top-bottom,design-only \
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/024-source.md,
...
```text
~/Desktop/xxd/xxd-panel-024/<fresh-task>/
└── source-01/
...
- the result follows `references/024-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-024-prompt.zh-CN.md`
- `references/xxd-panel-024-prompt.en.md`

These files document the same minimal adapter. They never replace `references/024-source.md`.
